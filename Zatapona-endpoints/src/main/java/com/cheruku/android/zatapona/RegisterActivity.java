...
import com.cheruku.android.zatapona.messageEndpoint.MessageEndpoint;
import com.cheruku.android.zatapona.messageEndpoint.model.CollectionResponseMessageData;
import com.cheruku.android.zatapona.messageEndpoint.model.MessageData;
import com.cheruku.android.zatapona.noteendpoint.Noteendpoint;
...
public class RegisterActivity extends Activity {
    ...
    private Noteendpoint noteEndpoint = null;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mSharedPreferencesEditor = getSharedPreferences(PREFS_NAME, 0).edit();
                        ...
                        if (GCMIntentService.PROJECT_NUMBER == null || GCMIntentService.PROJECT_NUMBER.length() == 0) {
                            ...
                            try {
                                ...
                                Log.e(RegisterActivity.class.getName(),
                                        "Exception received when attempting to register for Google Cloud Messaging. Perhaps you need to set your virtual device's target to Google APIs? See https://developers.google.com/eclipse/docs/cloud_endpoints_android for more information.", e);
        ...
        //initialize registration button properties
    ...
    private void initRegistrationButton() {
        ...
        if (getSharedPreferences(PREFS_NAME, 0).getBoolean(DEVICE_REGISTERED, false)) {
            mRegisterButton.setText("Unregister");
            curState = State.REGISTERED;
            mRegisterButton.setOnTouchListener(unregisterListener);
        } else {
            mRegisterButton.setText("Register");
            curState = State.UNREGISTERED;
            mRegisterButton.setOnTouchListener(registerListener);
        }
    ...
    private void updateState(State newState) {
        switch (newState) {
            case REGISTERED:
                ...
                mRegisterButton.setOnTouchListener(unregisterListener);
                mRegisterButton.setEnabled(true);
                mSharedPreferencesEditor.putBoolean(DEVICE_REGISTERED, true).commit();
                break;

            case REGISTERING:
                mRegisterButton.setText("Registering...");
            ...
            case UNREGISTERED:
                ...
                mRegisterButton.setOnTouchListener(registerListener);
                mRegisterButton.setEnabled(true);
                mSharedPreferencesEditor.putBoolean(DEVICE_REGISTERED, false).commit();
                break;

            case UNREGISTERING:
                mRegisterButton.setText("Unregistering...");
                mRegisterButton.setEnabled(false);
                break;
        }
        curState = newState;
